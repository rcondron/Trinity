...
function resolveTrinityRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.Trinity_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set Trinity_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveTrinityRoot(), "dist", "extensionAPI.js");
