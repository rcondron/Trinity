...
`Trinity.invoke --each`) without adding new Trinity code per workflow.
...
This extension depends on Trinity internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Trinity extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.Trinity/extensions` as a standalone plugin directory.
