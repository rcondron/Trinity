...
Trinity extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
Trinity channels login --channel zalouser
...
```bash
Trinity message send --channel zalouser --target <threadId> --message "Hello from Trinity!"
```
...
```bash
Trinity channels login --channel zalouser              # Login via QR
Trinity channels login --channel zalouser --account work
Trinity channels status --probe
Trinity channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
Trinity directory self --channel zalouser
Trinity directory peers list --channel zalouser --query "name"
Trinity directory groups list --channel zalouser --query "work"
Trinity directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
Trinity message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
Trinity message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `Trinity channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
Trinity channels login --channel zalouser --account work
Trinity message send --channel zalouser --account work --target <id> --message "Hello"
