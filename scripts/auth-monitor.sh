...
#
# Suggested cron: */30 * * * * /home/admin/Trinity/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Trinity notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., Trinity-alerts)
...
STATE_FILE="$HOME/.Trinity/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Trinity if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use Trinity
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Trinity to $NOTIFY_PHONE..."
            Trinity send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Trinity is down. Run: ssh l36 '~/Trinity/scripts/mobile-reauth.sh'" "urgent"
    exit 1
