#!/usr/bin/env bash
# ClawDock - Docker helpers for Trinity
# Inspired by Simon Willison's "Running Trinity in Docker"
# https://til.simonwillison.net/llms/Trinity-docker
#
# Installation:
#   mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/Trinity/Trinity/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
# Common paths to check for Trinity
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/Trinity"
  "${HOME}/workspace/Trinity"
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found Trinity at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/Trinity"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ Trinity not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/Trinity/Trinity.git ~/Trinity"
    echo "  cd ~/Trinity && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^Trinity_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d Trinity-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart Trinity-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f Trinity-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec Trinity-gateway \
    bash -c 'echo "alias Trinity=\"./Trinity.mjs\"" > /tmp/.bashrc_Trinity && bash --rcfile /tmp/.bashrc_Trinity'
}
...
clawdock-cli() {
  _clawdock_compose run --rm Trinity-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build Trinity-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "Trinity_GATEWAY_TOKEN=$token" Trinity-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" Trinity-gateway \
    bash -c './Trinity.mjs config set gateway.remote.token "$TOKEN" && ./Trinity.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec Trinity-gateway \
    bash -c "./Trinity.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart Trinity-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm Trinity-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec Trinity-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'Trinity config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for Trinity${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (Trinity alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to Trinity project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.Trinity)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'Trinity channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'Trinity status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.Trinity.ai${_CLR_RESET}"
  echo ""
}
