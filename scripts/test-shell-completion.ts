 ...
 * `Trinity update`. Use it to verify the completion installation flow
...
\`Trinity update\`, \`Trinity doctor\`, and \`Trinity onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# Trinity Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`Trinity completion --install\` later to enable.`));
    return;
  }
