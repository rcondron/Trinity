...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".Trinity");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.Trinity_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$Trinity_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via Trinity_STATE_DIR.
