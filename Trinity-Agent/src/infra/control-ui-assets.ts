...
import { resolveTrinityPackageRoot, resolveTrinityPackageRootSync } from "./Trinity-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveTrinityPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "Trinity" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveTrinityPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
