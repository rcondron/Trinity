...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: TrinityConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: TrinityConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: TrinityConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: TrinityConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: TrinityConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: TrinityConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: TrinityConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
