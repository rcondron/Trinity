...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.Trinity_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*Trinity-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "Trinity";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "Trinity");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "Trinity-gw");
  ...
  records.push(`_Trinity-gw._tcp IN PTR ${instanceLabel}._Trinity-gw._tcp`);
  records.push(`${instanceLabel}._Trinity-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._Trinity-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; Trinity-content-hash: ${contentHash}\n${contentBody}`;
}
