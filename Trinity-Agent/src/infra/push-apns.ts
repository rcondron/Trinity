...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.Trinity_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.Trinity_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set Trinity_APNS_TEAM_ID and Trinity_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.Trinity_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.Trinity_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.Trinity_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set Trinity_APNS_PRIVATE_KEY_P8 or Trinity_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading Trinity_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createTrinityPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    Trinity: createTrinityPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
