...
function candidateBinDirs(opts: EnsureTrinityPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `Trinity` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "Trinity");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.Trinity_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "Trinity"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `Trinity` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureTrinityCliOnPath(opts: EnsureTrinityPathOpts = {}) {
  if (isTruthyEnvValue(process.env.Trinity_PATH_BOOTSTRAPPED)) {
    return;
  }
