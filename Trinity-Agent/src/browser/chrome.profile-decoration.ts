...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".Trinity-profile-decorated");
}
 ...
 */
export function decorateTrinityProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_Trinity_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_Trinity_BROWSER_COLOR).toUpperCase();
