...
function stripUnknownConfigKeys(config: TrinityConfig): {
  config: TrinityConfig;
  removed: string[];
} {
  const parsed = TrinitySchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: TrinityConfig) {
...
function scanTelegramAllowFromUsernameEntries(cfg: TrinityConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: TrinityConfig): Promise<{
  config: TrinityConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: TrinityConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: TrinityConfig): {
  config: TrinityConfig;
  changes: string[];
} {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: TrinityConfig): {
  config: TrinityConfig;
  changes: string[];
} {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".Trinity");
  const targetPath = path.join(targetDir, "Trinity.json");
  try {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("Trinity doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("Trinity doctor --fix")}" to apply these changes.`);
    }
  }
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("Trinity doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("Trinity doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("Trinity doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "Trinity doctor --fix" to remove these keys.');
    }
  }
