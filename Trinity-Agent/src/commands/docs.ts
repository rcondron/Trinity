...
const SEARCH_TOOL = "https://docs.Trinity.ai/mcp.SearchTrinity";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.Trinity.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('Trinity docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.Trinity.ai/");
      runtime.log(`Search: ${formatCliCommand('Trinity docs "your query"')}`);
    }
    return;
  }
