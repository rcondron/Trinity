...
function summarizeGroupPolicy(cfg: TrinityConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: TrinityConfig): boolean {
...
function collectModels(cfg: TrinityConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: TrinityConfig): Set<string> {
  const baseCfg: TrinityConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: TrinityConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: TrinityConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: TrinityConfig): boolean {
...
function isBrowserEnabled(cfg: TrinityConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: TrinityConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: TrinityConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep Trinity_STATE_DIR on a local-only volume and re-run "${formatCliCommand("Trinity security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: TrinityConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer Trinity_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const TrinityGatewayToken =
    typeof env.Trinity_GATEWAY_TOKEN === "string" && env.Trinity_GATEWAY_TOKEN.trim()
      ? env.Trinity_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-Trinity-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: TrinityConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: TrinityConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: Trinity-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: TrinityConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: TrinityConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: TrinityConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: TrinityConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
