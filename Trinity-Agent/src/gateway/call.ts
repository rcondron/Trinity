...
export function buildGatewayConnectionDetails(
  options: { config?: TrinityConfig; url?: string; configPath?: string } = {},
...
function resolveGatewayCredentials(context: ResolvedGatewayCallContext): {
  ...
  const token =
    ...
    (!context.urlOverride
      ? context.isRemoteMode
        ...
        : trimToUndefined(process.env.Trinity_GATEWAY_TOKEN) ||
  ...
  const password =
    ...
    (!context.urlOverride
      ? trimToUndefined(process.env.Trinity_GATEWAY_PASSWORD) ||
