...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "Trinity_STATE_DIR",
  "Trinity_CONFIG_PATH",
  "Trinity_SKIP_BROWSER_CONTROL_SERVER",
  "Trinity_SKIP_GMAIL_WATCHER",
  "Trinity_SKIP_CANVAS_HOST",
  "Trinity_BUNDLED_PLUGINS_DIR",
  "Trinity_SKIP_CHANNELS",
  "Trinity_SKIP_PROVIDERS",
  "Trinity_SKIP_CRON",
  "Trinity_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "Trinity-gateway-home-"));
...
function applyGatewaySkipEnv() {
  process.env.Trinity_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.Trinity_SKIP_GMAIL_WATCHER = "1";
  process.env.Trinity_SKIP_CANVAS_HOST = "1";
  process.env.Trinity_SKIP_CHANNELS = "1";
  process.env.Trinity_SKIP_PROVIDERS = "1";
  process.env.Trinity_SKIP_CRON = "1";
  process.env.Trinity_TEST_MINIMAL_GATEWAY = "1";
  process.env.Trinity_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "Trinity-test-no-bundled-extensions")
    : "Trinity-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "Trinity-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".Trinity-test");
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["Trinity_GATEWAY_TOKEN"]);
  const prev = process.env.Trinity_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.Trinity_GATEWAY_TOKEN = fallbackToken;
  }
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.Trinity_GATEWAY_PASSWORD;
