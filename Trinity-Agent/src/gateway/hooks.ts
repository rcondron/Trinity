...
export function resolveHooksConfig(cfg: TrinityConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: TrinityConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-Trinity-token"] === "string"
      ? req.headers["x-Trinity-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
