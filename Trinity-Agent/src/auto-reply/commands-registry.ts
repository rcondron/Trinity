...
export function isCommandEnabled(cfg: TrinityConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: TrinityConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as TrinityConfig),
...
export function getCommandDetection(_cfg?: TrinityConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: TrinityConfig) {
