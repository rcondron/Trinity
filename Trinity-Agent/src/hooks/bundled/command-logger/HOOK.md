...
homepage: https://docs.Trinity.ai/automation/hooks#command-logger
metadata:
  {
    "Trinity":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.Trinity/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.Trinity/logs/commands.log ~/.Trinity/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/Trinity`:
   ```
   /home/username/.Trinity/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
...
```bash
tail -n 20 ~/.Trinity/logs/commands.log
```

Pretty-print with jq:

```bash
cat ~/.Trinity/logs/commands.log | jq .
```

Filter by action:

```bash
grep '"action":"new"' ~/.Trinity/logs/commands.log | jq .
```
