...
type WebSearchConfig = NonNullable<TrinityConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: TrinityConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("Trinity configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.Trinity.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://Trinity.ai",
      "X-Title": "Trinity Web Search",
    },
