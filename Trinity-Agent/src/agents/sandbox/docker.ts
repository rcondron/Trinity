...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "Trinity.sandbox=1");
  args.push("--label", `Trinity.sessionKey=${params.scopeKey}`);
  args.push("--label", `Trinity.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `Trinity.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "Trinity.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`Trinity sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`Trinity sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("Trinity sandbox recreate --all");
}
