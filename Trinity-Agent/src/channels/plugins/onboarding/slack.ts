...
function setSlackDmPolicy(cfg: TrinityConfig, dmPolicy: DmPolicy) {
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Trinity";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Trinity`,
    },
    features: {
      bot_user: {
      ...
      slash_commands: [
        {
          command: "/Trinity",
          description: "Send a message to Trinity",
          should_escape: false,
        },
      ],
    },
...
function setSlackAllowFrom(cfg: TrinityConfig, allowFrom: string[]): TrinityConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
