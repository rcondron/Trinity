...
import type { TrinityConfig } from "../config/types.js";
...
export const DEFAULT_LOG_DIR = resolvePreferredTrinityTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "Trinity.log"); // legacy single-file path

const LOG_PREFIX = "Trinity";
...
function resolveSettings(): ResolvedSettings {
  let cfg: TrinityConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => TrinityConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.Trinity_TEST_FILE_LOG !== "1" ? "silent" : "info";
