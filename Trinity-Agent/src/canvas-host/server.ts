...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.TrinityCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.TrinityCanvasA2UIAction &&
        typeof window.TrinityCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.TrinitySendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("Trinity:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Trinity node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.TrinitySendUserAction === "function"
        ? window.TrinitySendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.Trinity_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.Trinity_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Trinity Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
