...
declare const __Trinity_VERSION__: string | undefined;

const BUNDLED_VERSION =
  (typeof __Trinity_VERSION__ === "string" && __Trinity_VERSION__) ||
  process.env.Trinity_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `Trinity --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.Trinity_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.Trinity_GATEWAY_BIND ??
        ...
        if (isRestart) {
          ...
          if (respawn.mode === "spawned" || respawn.mode === "supervised") {
            ...
            if (respawn.mode === "failed") {
              ...
              defaultRuntime.log("gateway: restart mode in-process restart (Trinity_NO_RESPAWN)");
            }
            shuttingDown = false;
            restartResolver?.();
          }
