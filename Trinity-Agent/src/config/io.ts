...
import type { TrinityConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareTrinityVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "SLACK_BOT_TOKEN",
  "SLACK_APP_TOKEN",
  "Trinity_GATEWAY_TOKEN",
  "Trinity_GATEWAY_PASSWORD",
];
...
function coerceConfig(value: unknown): TrinityConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as TrinityConfig;
}
...
function stampConfigVersion(cfg: TrinityConfig): TrinityConfig {
...
function warnIfConfigFromFuture(cfg: TrinityConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareTrinityVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer Trinity (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as TrinityConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): TrinityConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as TrinityConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: TrinityConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfig =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as TrinityConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.Trinity_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.Trinity_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.Trinity_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.Trinity_WATCH_SESSION === "string" &&
        deps.env.Trinity_WATCH_SESSION.trim().length > 0
          ? deps.env.Trinity_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.Trinity_WATCH_COMMAND === "string" &&
        deps.env.Trinity_WATCH_COMMAND.trim().length > 0
          ? deps.env.Trinity_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `Trinity_CONFIG_PATH` (and friends) are expected to work even
...
function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.Trinity_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.Trinity_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): TrinityConfig {
