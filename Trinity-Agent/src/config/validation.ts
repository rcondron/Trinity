...
import type { TrinityConfig, ConfigValidationIssue } from "./types.js";
import { TrinitySchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: TrinityConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: TrinityConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = TrinitySchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as TrinityConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as TrinityConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as TrinityConfig,
  };
}
