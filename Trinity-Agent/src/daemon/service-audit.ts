...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.Trinity_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service Trinity_GATEWAY_TOKEN does not match gateway.auth.token in Trinity.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `Trinity gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
