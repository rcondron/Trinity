...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "Trinity";
...
export function registerBrowserManageCommands(
  ...
  browser
    ...
    .action(async (_opts, cmd) => {
      ...
      await runBrowserCommand(async () => {
        ...
        defaultRuntime.log(
          [
            `profile: ${status.profile ?? "Trinity"}`,
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (Trinity|extension). Default: Trinity. Use extension for the browser extension on port 9220.")
    .action(
