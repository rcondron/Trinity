...
import { resolveTrinityPackageRoot } from "../../infra/Trinity-root.js";
...
const Trinity_REPO_URL = "https://github.com/Trinity/Trinity.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "Trinity";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("Trinity@")) {
    return trimmed.slice("Trinity@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.Trinity_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveTrinityPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", Trinity_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `Trinity_GIT_DIR points at a non-git directory: ${params.dir}. Set Trinity_GIT_DIR to an empty folder or an Trinity checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`Trinity_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "Trinity.mjs");
