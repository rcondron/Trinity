    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["Trinity nodes status", "List known nodes with live status."],
          ["Trinity nodes pairing pending", "Show pending node pairing requests."],
          ['Trinity nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["Trinity nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.Trinity.ai/cli/nodes")}\n`,
    );
