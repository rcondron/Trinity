    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "Trinity node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["Trinity node status", "Check node host service status."],
          ["Trinity node install", "Install the node host service."],
          ["Trinity node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.Trinity.ai/cli/node")}\n`,
    );
