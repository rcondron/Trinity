    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["Trinity memory status", "Show index and provider status."],
          ["Trinity memory index --force", "Force a full reindex."],
          ['Trinity memory search --query "deployment notes"', "Search indexed memory entries."],
          ["Trinity memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.Trinity.ai/cli/memory")}\n`,
    );
