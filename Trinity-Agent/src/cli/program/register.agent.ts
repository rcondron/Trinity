...
${formatHelpExamples([
  ['Trinity agent --to +15555550123 --message "status update"', "Start a new session."],
  ['Trinity agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'Trinity agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'Trinity agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['Trinity agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'Trinity agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.Trinity.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.Trinity.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['Trinity agents set-identity --agent main --name "Trinity" --emoji "🦞"', "Set name + emoji."],
  ["Trinity agents set-identity --agent main --avatar avatars/Trinity.png", "Set avatar path."],
  [
    "Trinity agents set-identity --workspace ~/.Trinity/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "Trinity agents set-identity --identity-file ~/.Trinity/workspace/IDENTITY.md --agent main",
