...
function isCompletionProfileHeader(line: string): boolean {
  return line.trim() === "# Trinity Completion";
}
...
function isSlowDynamicCompletionLine(line: string, binName: string): boolean {
  // Matches patterns like: source <(Trinity completion --shell zsh)
  return (
...
function updateCompletionProfile(
  ...
  const block = `# Trinity Completion\n${sourceLine}`;
 ...
 * Returns true if profile has `source <(Trinity completion ...)` instead of cached file.
 */
...
export function registerCompletionCli(program: Command) {
  program
    ...
    .option(
      "--write-state",
      "Write completion scripts to $Trinity_STATE_DIR/completions (no stdout)",
    )
...
export async function installCompletion(shell: string, yes: boolean, binName = "Trinity") {
  const home = process.env.HOME || os.homedir();
  let profilePath = "";
  let sourceLine = "";
...
function generateFishCompletion(program: Command): string {
  ...
  const visit = (cmd: Command, parents: string[]) => {
    ...
    // For root: complete -c Trinity -n "__fish_use_subcommand" -a "subcmd" -d "desc"
    ...
    if (parents.length === 0) {
      ...
      // To properly scope to 'Trinity gateway' and not 'Trinity other gateway', we need to check the sequence.
