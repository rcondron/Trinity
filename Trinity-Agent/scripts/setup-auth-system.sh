#!/bin/bash
# Setup Trinity Auth Management System
...
echo "=== Trinity Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'Trinity-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Trinity message: Send warning via Trinity itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/Trinity-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/Trinity-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/Trinity-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now Trinity-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status Trinity-auth-monitor.timer"
echo ""
echo "Quick commands:"
