...
const shouldBuild = (deps) => {
  if (deps.env.MORPHEUS_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.MORPHEUS_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[morpheus] ${message}\n`);
};

const runMorpheus = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["morpheus.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
