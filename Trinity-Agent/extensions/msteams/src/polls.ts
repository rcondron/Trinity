...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["TrinityPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["Trinity", "pollId"]) ??
    readNestedString(value, ["Trinity", "poll", "id"]) ??
    readNestedString(value, ["data", "TrinityPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "Trinity", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          TrinityPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "Trinity poll vote",
          displayText: "Vote recorded",
          value: { TrinityPollId: pollId, pollId },
        },
      },
    ],
  };
