...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as TrinityConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as TrinityConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as TrinityConfig);
