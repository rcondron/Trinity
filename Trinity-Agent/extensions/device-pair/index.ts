...
import { approveDevicePairing, listDevicePairing } from "Trinity/plugin-sdk";
...
function resolveGatewayPort(cfg: TrinityPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.Trinity_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: TrinityPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: TrinityPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the Trinity iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the Trinity iOS app:",
            "",
            "```",
            qrAscii,
            "```",
