...
import type { TrinityPluginApi, TrinityPluginService } from "Trinity/plugin-sdk";
...
function normalizeDenyList(cfg: TrinityPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: TrinityPluginApi["config"]): string[] {
...
export default function register(api: TrinityPluginApi) {
  ...
  const timerService: TrinityPluginService = {
