...
metadata: { "Trinity": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
## Trinity Runtime Mapping

- **Task tool** in the upstream spec == Trinity `sessions_spawn`
- **File I/O** == Trinity `read`/`write`
- **Remote fetch** == Trinity `web_fetch` (or `exec` with curl when POST is required)
