/**
 * Twitch channel plugin for Trinity.
 *
...
import { buildChannelConfigSchema } from "Trinity/plugin-sdk";
 ...
 * for Trinity. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: TrinityConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: TrinityConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: TrinityConfig): boolean => {
