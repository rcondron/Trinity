...
describe("ensureExtensionRelayForProfiles", () => {
  ...
  it("does not start relay (extension uses direct port 9220)", async () => {
    resolveProfileMock.mockImplementation((_resolved: unknown, name: string) => {
      ...
      return { driver: "Trinity", cdpUrl: "http://127.0.0.1:18889" };
    });
...
describe("stopKnownBrowserProfiles", () => {
  ...
  it("stops all known profiles and ignores per-profile failures", async () => {
    listKnownProfileNamesMock.mockReturnValue(["Trinity", "chrome"]);
    ...
    expect(stopMap.Trinity).toHaveBeenCalledTimes(1);
  ...
  it("warns when profile enumeration fails", async () => {
    ...
    expect(onWarn).toHaveBeenCalledWith("Trinity browser stop failed: Error: oops");
  });
});
