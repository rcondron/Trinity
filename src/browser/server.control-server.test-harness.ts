...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  if (prevGatewayPort === undefined) {
    delete process.env.Trinity_GATEWAY_PORT;
    return;
  }
  process.env.Trinity_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/Trinity" }));

beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "Trinity-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "Trinity",
        profiles: {
          Trinity: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchTrinityChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveTrinityUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopTrinityChrome: vi.fn(async () => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.Trinity_GATEWAY_PORT;
    process.env.Trinity_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.Trinity_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.Trinity_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayToken === undefined) {
      ...
      process.env.Trinity_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.Trinity_GATEWAY_PASSWORD;
    } else {
      process.env.Trinity_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
