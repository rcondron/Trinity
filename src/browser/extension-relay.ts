...
const RELAY_AUTH_HEADER = "x-Trinity-relay-token";
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/Trinity-Extension-Relay",
          revision: "0",
          userAgent: "Trinity-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "Trinity/extension-relay",
  ...
  try {
    ...
    if (
      ...
      (await probeAuthenticatedTrinityRelay({
