...
describe("cdp.helpers", () => {
  ...
  it("does not add relay header for unknown loopback ports", () => {
    ...
    expect(headers["x-Trinity-relay-token"]).toBeUndefined();
  });
  ...
  it("adds relay header for known relay ports", async () => {
    ...
    process.env.Trinity_GATEWAY_TOKEN = "test-gateway-token";
    try {
      await ensureChromeExtensionRelayServer({ cdpUrl });
      const headers = getHeadersWithAuth(`${cdpUrl}/json/version`);
      expect(headers["x-Trinity-relay-token"]).toBeTruthy();
      expect(headers["x-Trinity-relay-token"]).not.toBe("test-gateway-token");
    } finally {
      await stopChromeExtensionRelayServer({ cdpUrl }).catch(() => {});
      if (prev === undefined) {
        delete process.env.Trinity_GATEWAY_TOKEN;
      } else {
        process.env.Trinity_GATEWAY_TOKEN = prev;
      }
    }
  });
});
...
describe("browser server-context listKnownProfileNames", () => {
  it("includes configured and runtime-only profile names", () => {
    const resolved = resolveBrowserConfig({
      ...
      profiles: {
        Trinity: { cdpPort: 18800, color: "#FF4500" },
      },
    });
    const Trinity = resolveProfile(resolved, "Trinity");
    if (!Trinity) {
      throw new Error("expected Trinity profile");
    }
    ...
    const state: BrowserServerState = {
      ...
      profiles: new Map([
        [
          "stale-removed",
          {
            profile: { ...Trinity, name: "stale-removed" },
            running: null,
          },
        ],
      ]),
