...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const Trinity = resolveProfile(resolved, "Trinity");
    expect(Trinity?.driver).toBe("Trinity");
    expect(Trinity?.cdpPort).toBe(18800);
    expect(Trinity?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from Trinity_GATEWAY_PORT when unset", () => {
    withEnv({ Trinity_GATEWAY_PORT: "19001" }, () => {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const Trinity = resolveProfile(resolved, "Trinity");
      expect(Trinity?.cdpPort).toBe(19012);
      expect(Trinity?.cdpUrl).toBe("http://127.0.0.1:19012");
    });
  });

  it("derives default ports from gateway.port when env is unset", () => {
    withEnv({ Trinity_GATEWAY_PORT: undefined }, () => {
      ...
      const Trinity = resolveProfile(resolved, "Trinity");
      expect(Trinity?.cdpPort).toBe(19022);
      expect(Trinity?.cdpUrl).toBe("http://127.0.0.1:19022");
    });
  });
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "Trinity");
  ...
  it("does not add the built-in chrome (TabHR) profile if port 9220 is already used", () => {
    ...
    expect(resolved.defaultProfile).toBe("Trinity");
  });
