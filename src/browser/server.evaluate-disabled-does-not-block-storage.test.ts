  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          Trinity: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.Trinity_GATEWAY_PORT;
    process.env.Trinity_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.Trinity_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.Trinity_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayToken === undefined) {
      ...
      process.env.Trinity_GATEWAY_TOKEN = prevGatewayToken;
    }
    if (prevGatewayPassword === undefined) {
      delete process.env.Trinity_GATEWAY_PASSWORD;
    } else {
      process.env.Trinity_GATEWAY_PASSWORD = prevGatewayPassword;
    }
