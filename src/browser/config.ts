import type { BrowserConfig, BrowserProfileConfig, TrinityConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_Trinity_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "Trinity" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_Trinity_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "Trinity" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_Trinity_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_Trinity_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  rootConfig?: TrinityConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_Trinity_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "Trinity";
