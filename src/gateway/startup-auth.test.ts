...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: TrinityConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: TrinityConfig) {
  ...
  it("throws when hooks token reuses gateway token resolved from env", async () => {
    await expect(
      ensureGatewayStartupAuth({
        ...
        env: {
          Trinity_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
