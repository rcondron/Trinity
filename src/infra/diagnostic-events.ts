...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __TrinityDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__TrinityDiagnosticEventsState) {
    globalStore.__TrinityDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__TrinityDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: TrinityConfig): boolean {
