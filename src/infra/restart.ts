...
export function triggerTrinityRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.Trinity_SYSTEMD_UNIT,
        process.env.Trinity_PROFILE,
      );
  ...
  const label =
    process.env.Trinity_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.Trinity_PROFILE);
