...
const DEFAULT_PACKAGE_NAME = "Trinity";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "Trinity-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "Trinity.mjs");
    ...
    const doctorStep = await runStep(
      step("Trinity doctor", doctorArgv, gitRoot, { Trinity_UPDATE_IN_PROGRESS: "1" }),
    );
