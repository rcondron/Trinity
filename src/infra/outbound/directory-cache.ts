...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: TrinityConfig | null = null;
  ...
  get(key: string, cfg: TrinityConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: TrinityConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: TrinityConfig): void {
