...
let ensureTrinityCliOnPath: typeof import("./path-env.js").ensureTrinityCliOnPath;

describe("ensureTrinityCliOnPath", () => {
  const envKeys = [
    "PATH",
    "Trinity_PATH_BOOTSTRAPPED",
    "Trinity_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureTrinityCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling Trinity exists", () => {
    const tmp = abs("/tmp/Trinity-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "Trinity");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.Trinity_PATH_BOOTSTRAPPED;

    ensureTrinityCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.Trinity_PATH_BOOTSTRAPPED = "1";
    ensureTrinityCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/Trinity-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "Trinity");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/Trinity-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "Trinity");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "Trinity");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/Trinity-path/case-linuxbrew");
