...
import { POSIX_Trinity_TMP_DIR, resolvePreferredTrinityTmpDir } from "./tmp-Trinity-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredTrinityTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `Trinity-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredTrinityTmpDir({
    accessSync,
...
describe("resolvePreferredTrinityTmpDir", () => {
  it("prefers /tmp/Trinity when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_Trinity_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/Trinity when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_Trinity_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/Trinity when /tmp/Trinity is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/Trinity when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/Trinity is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/Trinity is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/Trinity is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
