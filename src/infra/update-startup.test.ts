...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "Trinity-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["Trinity_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.Trinity_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveTrinityPackageRoot } = await import("./Trinity-root.js"));
  ...
  async function runUpdateCheckAndReadState(channel: "stable" | "beta") {
    vi.mocked(resolveTrinityPackageRoot).mockResolvedValue("/opt/Trinity");
