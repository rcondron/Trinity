...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "Trinity-version-"));
  try {
...
describe("version resolution", () => {
  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "Trinity", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withTempDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "Trinity", version: "2.3.4" });
  ...
  it("ignores non-Trinity package and blank build-info versions", async () => {
    await withTempDir(async (root) => {
  ...
  it("prefers Trinity_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        Trinity_VERSION: "9.9.9",
        Trinity_SERVICE_VERSION: "2.2.2",
