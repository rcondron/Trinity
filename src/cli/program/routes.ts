...
const routeBrainSetup: RouteSpec = {
  match: (path) => path[0] === "brain" && path[1] === "setup",
  run: async (argv) => {
    const force = hasFlag(argv, "--force");
    const skipHealthCheck = hasFlag(argv, "--skip-health-check");
    const portValue = getFlagValue(argv, "--port");
    const port = portValue ? parseInt(portValue, 10) : undefined;
    if (portValue && isNaN(port!)) {
      return false;
    }
    const { brainSetupCommand } = await import("../../commands/brain-setup.js");
    await brainSetupCommand({ force, skipHealthCheck, port }, defaultRuntime);
    return true;
  },
};
