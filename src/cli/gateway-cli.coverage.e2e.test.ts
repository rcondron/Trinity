...
describe("gateway-cli coverage", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      expectedOutput: [
        "Gateway Discovery",
        "Found 1 gateway(s)",
        "- Studio Trinity.internal.",
        "  tailnet: studio.tailnet.ts.net",
        "  host: studio.Trinity.internal",
        "  ws: ws://studio.Trinity.internal:18789",
      ],
    },
  ...
  it("uses env/config port when --port is omitted", async () => {
    await withEnvOverride({ Trinity_GATEWAY_PORT: "19001" }, async () => {
      resetRuntimeCapture();
