...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Trinity and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["Trinity update", "Update a source checkout (git)"],
        ["Trinity update --channel beta", "Switch to beta channel (git + npm)"],
        ["Trinity update --channel dev", "Switch to dev channel (git + npm)"],
        ["Trinity update --tag beta", "One-off update to a dist-tag or version"],
        ["Trinity update --no-restart", "Update without restarting the service"],
        ["Trinity update --json", "Output result as JSON"],
        ["Trinity update --yes", "Non-interactive (accept downgrade prompts)"],
        ["Trinity update wizard", "Interactive update wizard"],
        ["Trinity --update", "Shorthand for Trinity update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run Trinity update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.Trinity.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["Trinity update status", "Show channel + version status."],
          ["Trinity update status --json", "JSON output."],
          ["Trinity update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.Trinity.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
