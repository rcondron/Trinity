...
const SANDBOX_EXAMPLES = {
  main: [
    ["Trinity sandbox list", "List all sandbox containers."],
    ["Trinity sandbox list --browser", "List only browser containers."],
    ["Trinity sandbox recreate --all", "Recreate all containers."],
    ["Trinity sandbox recreate --session main", "Recreate a specific session."],
    ["Trinity sandbox recreate --agent mybot", "Recreate agent containers."],
    ["Trinity sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["Trinity sandbox list", "List all sandbox containers."],
    ["Trinity sandbox list --browser", "List only browser containers."],
    ["Trinity sandbox list --json", "JSON output."],
  ],
  recreate: [
    ["Trinity sandbox recreate --all", "Recreate all containers."],
    ["Trinity sandbox recreate --session main", "Recreate a specific session."],
    ["Trinity sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["Trinity sandbox recreate --browser --all", "Recreate only browser containers."],
    ["Trinity sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["Trinity sandbox explain", "Show effective sandbox config."],
    ["Trinity sandbox explain --session agent:main:main", "Explain a specific session."],
    ["Trinity sandbox explain --agent work", "Explain an agent sandbox."],
    ["Trinity sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.Trinity.ai/cli/sandbox")}\n`,
    )
    .action(() => {
