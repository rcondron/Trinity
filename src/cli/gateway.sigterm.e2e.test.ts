...
describe("gateway SIGTERM", () => {
  ...
  it("exits 0 on SIGTERM", { timeout: 180_000 }, async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "Trinity-gateway-test-"));
    const out: string[] = [];
    const err: string[] = [];
    ...
    const env = {
      ...process.env,
      Trinity_NO_RESPAWN: "1",
      Trinity_STATE_DIR: stateDir,
      Trinity_SKIP_CHANNELS: "1",
      Trinity_SKIP_GMAIL_WATCHER: "1",
      Trinity_SKIP_CRON: "1",
      Trinity_SKIP_BROWSER_CONTROL_SERVER: "1",
      Trinity_SKIP_CANVAS_HOST: "1",
    };
    const bootstrapPath = path.join(stateDir, "Trinity-entry-bootstrap.mjs");
