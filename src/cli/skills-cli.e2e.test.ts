...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "Trinity-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "Trinity-bundled-skills-test-"));
    process.env.Trinity_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
