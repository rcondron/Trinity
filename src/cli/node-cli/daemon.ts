...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("Trinity node install"), formatCliCommand("Trinity node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("Trinity node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    Trinity_LOG_PREFIX: baseEnv.Trinity_LOG_PREFIX ?? "node",
