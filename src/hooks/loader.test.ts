...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "Trinity-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["Trinity_BUNDLED_HOOKS_DIR"]);
    process.env.Trinity_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
