...
This directory contains hooks that ship with Trinity. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.Trinity/workspace`)

**Enable**:

```bash
Trinity hooks enable session-memory
```
...
```bash
Trinity hooks enable bootstrap-extra-files
```
...
**Output**: `~/.Trinity/logs/commands.log`

**Enable**:

```bash
Trinity hooks enable command-logger
```
...
homepage: https://docs.Trinity.ai/automation/hooks#my-hook
metadata:
  { "Trinity": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
- **Managed hooks**: `~/.Trinity/hooks/` (shared across workspaces)
...
```bash
Trinity hooks info session-memory
```
...
```bash
Trinity hooks enable session-memory
Trinity hooks disable command-logger
```

## Configuration

Hooks can be configured in `~/.Trinity/Trinity.json`:
...
2. Restart gateway: `pkill -9 -f 'Trinity.*gateway' && pnpm Trinity gateway`
3. Enable the hook: `Trinity hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.Trinity.ai/automation/hooks
