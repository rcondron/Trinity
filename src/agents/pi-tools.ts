...
function resolveExecConfig(params: { cfg?: TrinityConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: TrinityConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createTrinityCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createTrinityReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
