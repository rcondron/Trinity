...
const bundledPluginsDirSnapshot = captureEnv(["Trinity_BUNDLED_PLUGINS_DIR"]);

beforeAll(() => {
  process.env.Trinity_BUNDLED_PLUGINS_DIR = path.join(
    os.tmpdir(),
...
async function createSafeBinsExecTool(params: {
  ...
  const { createTrinityCodingTools } = await import("./pi-tools.js");
...
describe("createTrinityCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "Trinity-safe-bins-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        const marker = `safe-bins-${Date.now()}`;
        const result = await withEnvAsync(
          { Trinity_SHELL_ENV_TIMEOUT_MS: "1000" },
          async () =>
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "Trinity-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("does not leak file existence from sort output flags", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "Trinity-safe-bins-oracle-",
        safeBins: ["sort"],
  ...
  it("blocks sort output flags from writing files via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "Trinity-safe-bins-sort-",
        safeBins: ["sort"],
      },
      async ({ tmpDir, execTool }) => {
        const cases = [
  ...
  it("blocks sort --compress-program from bypassing safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "Trinity-safe-bins-sort-compress-",
        safeBins: ["sort"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "Trinity-safe-bins-redirect-",
        safeBins: ["head"],
  ...
  it("blocks grep recursive flags from reading cwd via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "Trinity-safe-bins-grep-",
        safeBins: ["grep"],
