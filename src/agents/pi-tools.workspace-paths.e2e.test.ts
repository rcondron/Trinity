...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("Trinity-ws-", async (workspaceDir) => {
      await withTempDir("Trinity-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createTrinityCodingTools({ workspaceDir });
    ...
    await withTempDir("Trinity-ws-", async (workspaceDir) => {
      await withTempDir("Trinity-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello Trinity");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("Trinity-ws-", async (workspaceDir) => {
      await withTempDir("Trinity-override-", async (overrideDir) => {
        const tools = createTrinityCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("Trinity-sandbox-", async (sandboxDir) => {
      await withTempDir("Trinity-workspace-", async (workspaceDir) => {
        ...
        const tools = createTrinityCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
