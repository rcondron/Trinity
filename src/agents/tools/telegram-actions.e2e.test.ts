...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): TrinityConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as TrinityConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): TrinityConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as TrinityConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: TrinityConfig, accountId = "media") {
    await handleTelegramAction(
