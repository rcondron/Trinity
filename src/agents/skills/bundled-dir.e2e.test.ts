...
describe("resolveBundledSkillsDir", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["Trinity_BUNDLED_SKILLS_DIR"]);
  });
  ...
  it("returns Trinity_BUNDLED_SKILLS_DIR override when set", async () => {
    const overrideDir = await fs.mkdtemp(path.join(os.tmpdir(), "Trinity-bundled-override-"));
    process.env.Trinity_BUNDLED_SKILLS_DIR = ` ${overrideDir} `;
    expect(resolveBundledSkillsDir()).toBe(overrideDir);
  });

  it("resolves bundled skills under a flattened dist layout", async () => {
    delete process.env.Trinity_BUNDLED_SKILLS_DIR;

    const root = await fs.mkdtemp(path.join(os.tmpdir(), "Trinity-bundled-"));
    await fs.writeFile(path.join(root, "package.json"), JSON.stringify({ name: "Trinity" }));
