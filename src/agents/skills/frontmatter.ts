...
import {
  ...
  normalizeStringList,
  parseTrinityManifestInstallBase,
  parseFrontmatterBool,
  resolveTrinityManifestBlock,
  resolveTrinityManifestInstall,
  resolveTrinityManifestOs,
  resolveTrinityManifestRequires,
} from "../../shared/frontmatter.js";
import type {
  TrinitySkillMetadata,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseTrinityManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
...
export function resolveTrinityMetadata(
  frontmatter: ParsedSkillFrontmatter,
): TrinitySkillMetadata | undefined {
  const metadataObj = resolveTrinityManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveTrinityManifestRequires(metadataObj);
  const install = resolveTrinityManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveTrinityManifestOs(metadataObj);
  return {
