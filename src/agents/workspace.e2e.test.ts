...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses Trinity_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      Trinity_HOME: "/srv/Trinity-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/Trinity-home"), ".Trinity", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".Trinity", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("Trinity-workspace-");
