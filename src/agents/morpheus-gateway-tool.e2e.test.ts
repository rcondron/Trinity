...
describe("gateway tool", () => {
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "Trinity-test-"));

    try {
      await withEnvAsync(
        { Trinity_STATE_DIR: stateDir, Trinity_PROFILE: "isolated" },
        async () => {
          const tool = createTrinityTools({
            config: { commands: { restart: true } },
          }).find((candidate) => candidate.name === "gateway");
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: Trinity --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/Trinity" } }\n}\n';
