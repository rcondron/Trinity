...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["Trinity_STATE_DIR"]);
  let tempStateDir: string | null = null;

  const writePersistedRegistry = async (persisted: Record<string, unknown>) => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "Trinity-subagent-"));
  ...
  it("uses isolated temp state when Trinity_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "Trinity-test-state"));
  });
});
