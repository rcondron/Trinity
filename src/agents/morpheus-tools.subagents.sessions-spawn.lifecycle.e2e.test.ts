...
} from "./Trinity-tools.subagents.sessions-spawn.test-harness.js";
...
type CreateTrinityTools = (typeof import("./Trinity-tools.js"))["createTrinityTools"];
type CreateTrinityToolsOpts = Parameters<CreateTrinityTools>[0];

async function getSessionsSpawnTool(opts: CreateTrinityToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createTrinityTools } = await import("./Trinity-tools.js");
  const tool = createTrinityTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("Trinity-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
