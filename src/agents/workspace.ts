...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.Trinity_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".Trinity", `workspace-${profile}`);
  }
  return path.join(home, ".Trinity", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".Trinity";
