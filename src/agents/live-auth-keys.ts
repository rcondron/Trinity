...
const GOOGLE_LIVE_SINGLE_KEY = "Trinity_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "Trinity_LIVE_ANTHROPIC_KEY",
    listVar: "Trinity_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "Trinity_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `Trinity_LIVE_${base}_KEY`;
