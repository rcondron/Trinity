...
const defaultTools = createTrinityCodingTools({ senderIsOwner: true });
...
describe("createTrinityCodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    ...
    const openAiTools = createTrinityCodingTools({
      config,
    ...
    const anthropicTools = createTrinityCodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createTrinityCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = createTrinityCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
