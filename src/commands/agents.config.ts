...
function resolveAgentName(cfg: TrinityConfig, agentId: string) {
...
function resolveAgentModel(cfg: TrinityConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: TrinityConfig): AgentSummary[] {
