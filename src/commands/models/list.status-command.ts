...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveTrinityAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("Trinity models auth setup-token")}\` or \`${formatCliCommand("Trinity configure")}\`.`
          : `Run \`${formatCliCommand("Trinity configure")}\` or set an API key env var.`;
