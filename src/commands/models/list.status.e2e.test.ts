...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveTrinityAgentDir: vi.fn().mockReturnValue("/tmp/Trinity-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/Trinity-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/Trinity-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveTrinityAgentDir: mocks.resolveTrinityAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/Trinity-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveTrinityAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/Trinity-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/Trinity-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/Trinity-agent-custom");
