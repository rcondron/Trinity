...
export async function loadValidConfigOrThrow(): Promise<TrinityConfig> {
...
export async function updateConfig(
  mutator: (cfg: TrinityConfig) => TrinityConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: TrinityConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: TrinityConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("Trinity agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
