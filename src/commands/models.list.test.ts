...
const ensureTrinityModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/Trinity-agent/auth.json" });
const resolveTrinityAgentDir = vi.fn().mockReturnValue("/tmp/Trinity-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/Trinity-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureTrinityModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/Trinity-agent");
  });
