...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: TrinityConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: TrinityConfig) => { changed: boolean; next: TrinityConfig };
  defaultModel: string;
  overrideConfig: TrinityConfig;
  alreadyDefaultConfig: TrinityConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: TrinityConfig) => config,
      applyDefaultConfig: (config: TrinityConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: TrinityConfig = { agents: { defaults: {} } };
