...
function resolveMode(cfg: TrinityConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Trinity doctor");

  const root = await resolveTrinityPackageRoot({
  ...
  let cfg: TrinityConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("Trinity configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("Trinity config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("Trinity setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("Trinity doctor --fix")}" to apply changes.`);
  }
