...
beforeAll(() => {
  envSnapshot = captureEnv(["Trinity_PROFILE"]);
  process.env.Trinity_PROFILE = "isolated";
});
...
vi.mock("../infra/Trinity-root.js", () => ({
  resolveTrinityPackageRoot: vi.fn().mockResolvedValue("/tmp/Trinity"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/Trinity/pnpm-lock.yaml",
      markerPath: "/tmp/Trinity/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("Trinity status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("Trinity status --all") ||
          l.includes("Trinity --profile isolated status --all") ||
          l.includes("Trinity status --all") ||
          l.includes("Trinity --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.Trinity_GATEWAY_TOKEN = "abcd1234";
    try {
