...
export const resolveTrinityPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadTrinityPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveTrinityPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.Trinity_STATE_DIR;
  originalUpdateInProgress = process.env.Trinity_UPDATE_IN_PROGRESS;
  process.env.Trinity_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "Trinity-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.Trinity_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.Trinity_UPDATE_IN_PROGRESS;
  } else {
    process.env.Trinity_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
