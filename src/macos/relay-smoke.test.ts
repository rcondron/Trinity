...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { Trinity_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { Trinity_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports Trinity_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { Trinity_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { Trinity_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { Trinity_SMOKE: "nope" })).toBe("qr");
  });
});
