...
describe("shared/frontmatter", () => {
  ...
  test("resolveTrinityManifestBlock parses JSON5 metadata and picks Trinity block", () => {
    const frontmatter = {
      metadata: "{ Trinity: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveTrinityManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveTrinityManifestBlock returns undefined for invalid input", () => {
    expect(resolveTrinityManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveTrinityManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveTrinityManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
