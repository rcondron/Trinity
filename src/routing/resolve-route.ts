...
function listAgents(cfg: TrinityConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: TrinityConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: TrinityConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<TrinityConfig, EvaluatedBindingsCache>();
