...
function telegramCfg(): TrinityConfig {
  return { channels: { telegram: { botToken: "tok" } } } as TrinityConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: TrinityConfig; accountId?: string },
) {
...
async function runSignalAction(
  ...
  const cfg =
    options?.cfg ?? ({ channels: { signal: { account: "+15550001111" } } } as TrinityConfig);
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as TrinityConfig;
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as TrinityConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as TrinityConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
  ...
  it("maps reaction targets into signal sendReaction calls", async () => {
    const cases = [
      ...
      },
      {
        ...
        cfg: { channels: { signal: { account: "+15550001111" } } } as TrinityConfig,
        accountId: undefined,
        params: {
