...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("Trinity-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("Trinity-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("Trinity-lid-a-", (first) => {
      withTempDirSync("Trinity-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.Trinity when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "Trinity-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers Trinity_HOME over HOME", () => {
    vi.stubEnv("Trinity_HOME", "/srv/Trinity-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/Trinity-home"));
...
describe("shortenHomePath", () => {
  it("uses $Trinity_HOME prefix when Trinity_HOME is set", () => {
    vi.stubEnv("Trinity_HOME", "/srv/Trinity-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/Trinity-home")}/.Trinity/Trinity.json`)).toBe(
      "$Trinity_HOME/.Trinity/Trinity.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $Trinity_HOME replacement when Trinity_HOME is set", () => {
    vi.stubEnv("Trinity_HOME", "/srv/Trinity-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/Trinity-home")}/.Trinity/Trinity.json`),
    ).toBe("config: $Trinity_HOME/.Trinity/Trinity.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/Trinity")).toBe(path.resolve(os.homedir(), "Trinity"));
  });
  ...
  it("prefers Trinity_HOME for tilde expansion", () => {
    vi.stubEnv("Trinity_HOME", "/srv/Trinity-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/Trinity")).toBe(path.resolve("/srv/Trinity-home", "Trinity"));
