...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.Trinity_GATEWAY_PORT).toBe("18789");
    expect(env.Trinity_GATEWAY_TOKEN).toBe("secret");
    expect(env.Trinity_SERVICE_MARKER).toBe("Trinity");
    expect(env.Trinity_SERVICE_KIND).toBe("gateway");
    expect(typeof env.Trinity_SERVICE_VERSION).toBe("string");
    expect(env.Trinity_SYSTEMD_UNIT).toBe("Trinity-gateway.service");
    if (process.platform === "darwin") {
      expect(env.Trinity_LAUNCHD_LABEL).toBe("ai.Trinity.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", Trinity_PROFILE: "work" },
      port: 18789,
    });
    expect(env.Trinity_SYSTEMD_UNIT).toBe("Trinity-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.Trinity_LAUNCHD_LABEL).toBe("ai.Trinity.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".Trinity"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", Trinity_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".Trinity-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", Trinity_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".Trinity"));
  });

  it("uses Trinity_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", Trinity_STATE_DIR: "/var/lib/Trinity" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/Trinity"));
  });

  it("expands ~ in Trinity_STATE_DIR", () => {
    const env = { HOME: "/Users/test", Trinity_STATE_DIR: "~/Trinity-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/Trinity-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { Trinity_STATE_DIR: "C:\\State\\Trinity" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\Trinity");
  });
});
