...
describe("discord allowlist helpers", () => {
  it("normalizes slugs", () => {
    expect(normalizeDiscordSlug("Friends of Trinity")).toBe("friends-of-Trinity");
  ...
  it("matches ids or names", () => {
    const allow = normalizeDiscordAllowList(
      ["123", "steipete", "Friends of Trinity"],
    ...
    expect(allowListMatches(allow, { name: "friends-of-Trinity" })).toBe(true);
...
describe("discord guild/channel resolution", () => {
  it("resolves guild entry by id", () => {
    const guildEntries = makeEntries({
      "123": { slug: "friends-of-Trinity" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Trinity"),
      guildEntries,
    });
    expect(resolved?.id).toBe("123");
    expect(resolved?.slug).toBe("friends-of-Trinity");
  });

  it("resolves guild entry by slug key", () => {
    const guildEntries = makeEntries({
      "friends-of-Trinity": { slug: "friends-of-Trinity" },
    });
    const resolved = resolveDiscordGuildEntry({
      guild: fakeGuild("123", "Friends of Trinity"),
      guildEntries,
  ...
  it("matches group DM allowlist", () => {
    expect(
      resolveGroupDmAllow({
        channels: ["Trinity-dm"],
        channelId: "1",
        channelName: "Trinity DM",
        channelSlug: "Trinity-dm",
      }),
    ).toBe(true);
    expect(
      resolveGroupDmAllow({
