...
describe("thread binding ttl", () => {
  ...
  it("persists unbinds even when no manager is active", () => {
    const previousStateDir = process.env.Trinity_STATE_DIR;
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "Trinity-thread-bindings-"));
    ...
    try {
      ...
      if (previousStateDir === undefined) {
        ...
        process.env.Trinity_STATE_DIR = previousStateDir;
      }
