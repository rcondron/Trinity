...
function resolveVoiceTtsConfig(params: { cfg: TrinityConfig; override?: TtsConfig }): {
  cfg: TrinityConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredTrinityTmpDir(), "discord-voice-"));
