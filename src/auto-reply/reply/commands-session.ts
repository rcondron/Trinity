...
import { scheduleGatewaySigusr1Restart, triggerTrinityRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Trinity in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerTrinityRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Trinity via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
