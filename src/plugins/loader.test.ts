...
import { loadTrinityPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `Trinity-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.Trinity_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "Trinity.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.Trinity_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadTrinityPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.Trinity_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.Trinity_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadTrinityPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadTrinityPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadTrinityPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.Trinity_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ Trinity_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
