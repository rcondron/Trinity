...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `Trinity-plugin-install-${randomUUID()}`);
    ...
    JSON.stringify(
      {
        ...
        Trinity: { extensions: params.extensions },
      },
      null,
      2,
    ),
...
async function expectArchiveInstallReservedSegmentRejection(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      version: "0.0.1",
      Trinity: { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  );
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.Trinity/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without Trinity.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@Trinity/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("Trinity.extensions");
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@Trinity/voice-call@0.0.1");
    expect(result.npmResolution?.integrity).toBe("sha512-plugin-test");

    expectSingleNpmPackIgnoreScriptsCall({
      calls: run.mock.calls,
      expectedSpec: "@Trinity/voice-call@0.0.1",
    });
