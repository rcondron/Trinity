...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: TrinityPluginDefinition;
  register?: TrinityPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as TrinityPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as TrinityPluginDefinition;
...
export function loadTrinityPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverTrinityPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "Trinity/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "Trinity/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: TrinityPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as TrinityPluginModule;
    } catch (err) {
