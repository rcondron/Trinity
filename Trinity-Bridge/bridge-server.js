...
function createServer(cfg, token, trinity, brain) {
  ...
  // ---- Test runner (Trinity-Tests) -------------------------------------------

  route("POST", "/tests/count", async (req) => {
    requireAuth(req);
    return new Promise((resolve) => {
      const testsDir = path.resolve(__dirname, "..", "Trinity-Tests");
      execFile("find", [testsDir, "-name", "*.test.ts", "-type", "f"], { timeout: 10_000 }, (err, stdout) => {
  ...
  route("POST", "/tests/run", async (req) => {
    requireAuth(req);
    const body = await readBody(req) || {};
    const suite = String(body.suite || "all").replace(/[^a-z0-9._-]/gi, "");
    const agentDir = path.resolve(__dirname, "..", "Trinity-Agent");
    const testsDir = path.resolve(__dirname, "..", "Trinity-Tests");
    const configFile = suite === "all"
      ? path.join(testsDir, "vitest.config.ts")
      : path.join(testsDir, `vitest.${suite}.config.ts`);

    audit("tests.run", { suite });
    return new Promise((resolve) => {
      execFile("npx", ["vitest", "run", "--config", configFile], {
        cwd: agentDir,
        timeout: 600_000,
  ...
  function dockerCompose(...args) {
    return new Promise((resolve) => {
      const composeFile = path.resolve(__dirname, "..", "Trinity-WebUI", "docker", "docker-compose.trinity.yml");
