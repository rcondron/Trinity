...
const loadTrinityPluginsMock = vi.fn();

vi.mock("./loader.js", () => ({
  loadTrinityPlugins: (params: unknown) => loadTrinityPluginsMock(params),
}));
...
function setRegistry(entries: MockRegistryToolEntry[]) {
  ...
  loadTrinityPluginsMock.mockReturnValue(registry);
  return registry;
}

describe("resolvePluginTools optional tools", () => {
  beforeEach(() => {
    loadTrinityPluginsMock.mockReset();
  });
