...
`;
    ...
    expect(parsed.Trinity.emoji).toBe("💾");
    expect(parsed.Trinity.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.Trinity.emoji).toBe("📝");
    expect(parsed.Trinity.events).toEqual(["command"]);
    expect(parsed.Trinity.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.Trinity.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"Trinity": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"Trinity": {"events": ["test"]}}');
  });
...
describe("resolveTrinityMetadata", () => {
  it("extracts Trinity metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveTrinityMetadata(frontmatter);
  ...
  it("returns undefined when Trinity key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        Trinity: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Trinity" },
            { id: "npm", kind: "npm", package: "@Trinity/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@Trinity/hook");
  });
...
homepage: https://docs.Trinity.ai/automation/hooks#session-memory
metadata:
  {
    "Trinity":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Trinity" }],
      },
  }
---
...
`;
    ...
    const Trinity = resolveTrinityMetadata(frontmatter);
    expect(Trinity).toBeDefined();
    expect(Trinity?.emoji).toBe("💾");
    expect(Trinity?.events).toEqual(["command:new", "command:reset"]);
    expect(Trinity?.requires?.config).toEqual(["workspace.dir"]);
    expect(Trinity?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const Trinity = resolveTrinityMetadata(frontmatter);
    expect(Trinity?.emoji).toBe("disk");
    expect(Trinity?.events).toEqual(["command:new"]);
  });
});
