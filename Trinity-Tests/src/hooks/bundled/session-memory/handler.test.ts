...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => TrinityConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("Trinity-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): TrinityConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
