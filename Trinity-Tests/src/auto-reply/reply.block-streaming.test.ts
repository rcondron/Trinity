...
function createReplyConfig(home: string, streamMode?: "block"): TrinityConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("Trinity-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("Trinity_TEST_FAST", "1");
