...
function cfg(accounts?: Record<string, unknown> | null): TrinityConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as TrinityConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as TrinityConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as TrinityConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as TrinityConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as TrinityConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as TrinityConfig)).toBe("default");
    });
  });
});
