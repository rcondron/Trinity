...
describe("resolveEffectiveHomeDir", () => {
  it("prefers Trinity_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands Trinity_HOME when set to ~", () => {
    const env = {
      Trinity_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for Trinity_HOME", () => {
    const result = resolveRequiredHomeDir(
      { Trinity_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when Trinity_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ Trinity_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { Trinity_HOME: "/srv/Trinity-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/Trinity-home")}/x`);
  });
