...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__Trinity_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "Trinity-root");
...
describe("resolveTrinityPackageRoot", () => {
  let resolveTrinityPackageRoot: typeof import("./Trinity-root.js").resolveTrinityPackageRoot;
  let resolveTrinityPackageRootSync: typeof import("./Trinity-root.js").resolveTrinityPackageRootSync;

  beforeAll(async () => {
    ({ resolveTrinityPackageRoot, resolveTrinityPackageRootSync } =
      await import("./Trinity-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "Trinity");
    const pkgRoot = path.join(project, "node_modules", "Trinity");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "Trinity" }));

    expect(resolveTrinityPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "Trinity");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "Trinity.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "Trinity" }));

    expect(resolveTrinityPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveTrinityPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-Trinity package roots", async () => {
    const pkgRoot = fx("not-Trinity");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-Trinity" }));

    expect(resolveTrinityPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveTrinityPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveTrinityPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
