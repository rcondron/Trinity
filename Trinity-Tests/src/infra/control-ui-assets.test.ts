...
vi.mock("./Trinity-root.js", () => ({
  resolveTrinityPackageRoot: vi.fn(async () => null),
  resolveTrinityPackageRootSync: vi.fn(() => null),
}));
...
let TrinityRoot: typeof import("./Trinity-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    TrinityRoot = await import("./Trinity-root.js");
  });
  ...
  it("uses resolveTrinityPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/Trinity");
    (
      TrinityRoot.resolveTrinityPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/Trinity"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "Trinity" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "Trinity.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-Trinity");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/Trinity-bundle");
    (
      TrinityRoot.resolveTrinityPackageRootSync as unknown as ReturnType<typeof vi.fn>
