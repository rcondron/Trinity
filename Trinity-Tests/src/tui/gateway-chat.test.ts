...
describe("resolveGatewayConnection", () => {
  ...
  it("uses Trinity_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ Trinity_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers Trinity_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ Trinity_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
