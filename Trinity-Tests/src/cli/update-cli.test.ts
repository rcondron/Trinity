...
import type { TrinityConfig, ConfigFileSnapshot } from "../config/types.Trinity.js";
...
vi.mock("../infra/Trinity-root.js", () => ({
  resolveTrinityPackageRoot: vi.fn(),
}));
...
const { resolveTrinityPackageRoot } = await import("../infra/Trinity-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/Trinity-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as TrinityConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/Trinity-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveTrinityPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("Trinity-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveTrinityPackageRoot).mockReset();
    ...
    vi.mocked(resolveTrinityPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("Trinity");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/Trinity-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "Trinity-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Trinity update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as TrinityConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveTrinityPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("Trinity-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ Trinity_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        ...
        expect(process.env.Trinity_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("Trinity-update-wizard");
    await withEnvAsync({ Trinity_GIT_DIR: tempDir }, async () => {
      setTty(true);
