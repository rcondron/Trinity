...
beforeEach(() => {
  ...
  originalHideBanner = process.env.Trinity_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.Trinity_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.Trinity_HIDE_BANNER = originalHideBanner;
  }
});

describe("registerPreActionHooks", () => {
  function buildProgram() {
    const program = new Command().name("Trinity");
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "Trinity", "status", "--debug"],
    });
  ...
  it("skips config guard for doctor and completion commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "Trinity", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "Trinity", "completion"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "Trinity", "--version"],
    });
  ...
  it("hides banner when Trinity_HIDE_BANNER is truthy", async () => {
    process.env.Trinity_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "Trinity", "status"],
    });
