...
describe("registerSubCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.Trinity_DISABLE_LAZY_SUBCOMMANDS;
  ...
  it("registers only the primary placeholder and dispatches", async () => {
    const program = createRegisteredProgram(["node", "Trinity", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "Trinity"]);
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "Trinity", "nodes", "list"], "Trinity");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "Trinity", "acp", "--help"], "Trinity");
    ...
    await program.parseAsync(["node", "Trinity", "acp"], { from: "user" });
    expect(registerAcpCli).toHaveBeenCalledTimes(1);
    expect(acpAction).toHaveBeenCalledTimes(1);
  });
});
