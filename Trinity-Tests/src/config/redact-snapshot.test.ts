...
import type { ConfigFileSnapshot } from "./types.Trinity.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.Trinity/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/Trinity/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/Trinity/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/Trinity/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/Trinity/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = TrinitySchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "TrinityConfig";
    const hints = mapSensitivePaths(TrinitySchema, "", {});
