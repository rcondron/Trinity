...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("Trinity-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "Trinity-sandbox-browser-global" },
      agentBrowser: { network: "Trinity-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("Trinity-sandbox-browser-agent");
  });
