...
describe("canvas host state dir defaults", () => {
  it("uses Trinity_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("Trinity-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Trinity Canvas");
      } finally {
        await handler.close();
      }
    });
