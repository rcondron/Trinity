...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "Trinity-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses Trinity_HOME for tilde expansion", () => {
    vi.stubEnv("Trinity_HOME", "/srv/Trinity-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/Trinity-home", "cron", "jobs.json"));
  });
});
