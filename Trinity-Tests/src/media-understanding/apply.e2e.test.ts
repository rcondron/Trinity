...
const TEMP_MEDIA_PREFIX = "Trinity-media-";
const tempMediaDirs: string[] = [];

async function createTempMediaDir() {
  const baseDir = resolvePreferredTrinityTmpDir();
...
function createGroqAudioConfig(): TrinityConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): TrinityConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
