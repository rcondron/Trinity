...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredTrinityTmpDir: vi.fn(() => "/tmp/Trinity"),
}));
vi.mock("../infra/tmp-Trinity-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredTrinityTmpDir.mockReturnValue("/tmp/Trinity");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredTrinityTmpDir.mockReturnValue("/tmp/Trinity-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "Trinity-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredTrinityTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "Trinity-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "Trinity-preferred", "downloads")}${path.sep}`),
    );
  });
