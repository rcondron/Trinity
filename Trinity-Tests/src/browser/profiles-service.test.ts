...
vi.mock("./chrome.js", () => ({
  resolveTrinityUserDataDir: vi.fn(() => "/tmp/Trinity-test/Trinity/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveTrinityUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "Trinity-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveTrinityUserDataDir).mockReturnValue(userDataDir);
