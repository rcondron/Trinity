...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "Trinity-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "Trinity");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.Trinity-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".Trinity");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "Trinity.json");
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "Trinity-wizard-home-"));
      process.env.HOME = tempHome;
      delete process.env.Trinity_STATE_DIR;
