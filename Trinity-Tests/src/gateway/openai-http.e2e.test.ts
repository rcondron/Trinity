    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "Trinity", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-Trinity-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-Trinity-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          ...
          { model: "Trinity", messages: [{ role: "user", content: "hi" }] },
          {
            ...
            "x-Trinity-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
