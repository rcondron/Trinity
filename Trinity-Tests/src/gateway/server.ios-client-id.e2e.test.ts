...
beforeAll(async () => {
  previousToken = process.env.Trinity_GATEWAY_TOKEN;
  process.env.Trinity_GATEWAY_TOKEN = "test-gateway-token-1234567890";
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.Trinity_GATEWAY_TOKEN = previousToken;
  }
});
...
test.each([
  { clientId: "Trinity-ios", platform: "ios" },
  { clientId: "Trinity-android", platform: "android" },
