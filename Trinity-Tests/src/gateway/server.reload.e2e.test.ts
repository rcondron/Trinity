...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.Trinity_SKIP_CHANNELS;
    prevSkipGmail = process.env.Trinity_SKIP_GMAIL_WATCHER;
    prevSkipProviders = process.env.Trinity_SKIP_PROVIDERS;
    process.env.Trinity_SKIP_CHANNELS = "0";
    delete process.env.Trinity_SKIP_GMAIL_WATCHER;
    delete process.env.Trinity_SKIP_PROVIDERS;
  });
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.Trinity_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.Trinity_SKIP_GMAIL_WATCHER;
    } else {
      process.env.Trinity_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
    if (prevSkipProviders === undefined) {
      delete process.env.Trinity_SKIP_PROVIDERS;
    } else {
      process.env.Trinity_SKIP_PROVIDERS = prevSkipProviders;
    }
  });
