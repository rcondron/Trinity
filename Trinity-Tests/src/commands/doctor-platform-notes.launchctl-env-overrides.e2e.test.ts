...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "Trinity_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("Trinity_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv Trinity_GATEWAY_TOKEN");
    expect(message).not.toContain("Trinity_GATEWAY_PASSWORD");
  });
