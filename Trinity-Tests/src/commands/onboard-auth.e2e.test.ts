...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under Trinity_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("Trinity-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "Trinity-oauth-sync-"));
    ...
    process.env.Trinity_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "Trinity-oauth-nosync-"));
    process.env.Trinity_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside Trinity_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "Trinity-oauth-external-"));
    process.env.Trinity_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* Trinity_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to Trinity_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("Trinity-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
